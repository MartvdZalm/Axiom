...
core::Result<SemanticModel> SemanticAnalyzer::analyze(std::uint32_t buffer_id,
...
void SemanticAnalyzer::analyze_machine(std::uint32_t buffer_id, const ast::MachineDecl& machine,
                                       SemanticModel& out)
{
    ...
    for (std::size_t i = 0; i < machine.states.size(); ++i)
    {
        ...
        {
            diagnostics_.add_error(buffer_id, state.span.begin,
    ...
    for (const auto& transition : machine.transitions)
    {
        if (!table.has_state(transition.source_state))
        {
            diagnostics_.add_error(
                buffer_id, transition.span.begin,
